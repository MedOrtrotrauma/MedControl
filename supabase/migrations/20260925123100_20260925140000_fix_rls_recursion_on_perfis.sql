/*
# Corrigir recursão nas políticas de perfis_usuario

1. Problema
- As políticas de SELECT/INSERT/UPDATE/DELETE em `perfis_usuario` fazem subconsulta na própria tabela, gerando recursão infinita e erro 500.

2. Solução
- Criar função `is_current_user_admin()` SECURITY DEFINER que verifica o perfil sem disparar RLS.
- Reescrever todas as políticas de `perfis_usuario` e das tabelas de cadastro/repasses para usar essa função.
- Reescrever a política de SELECT de `perfis_usuario` para permitir que cada usuário leia seu próprio perfil sem recursão.

3. Observações
- Nenhum dado é alterado ou apagado.
- A função bypassa RLS intencionalmente, pois precisa ler perfis sem recursão.
*/
...
-- Repasses: usar função em vez de subconsulta
